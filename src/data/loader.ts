...
const BUNDLER_LAYOUT_GLOB = import.meta.glob<{ default: unknown }>('../../data/layouts/*.json');
const BUNDLER_LANGUAGE_GLOB = import.meta.glob<{ default: unknown }>('../../data/languages/*.json');
const BUNDLER_REGISTRY_GLOB = import.meta.glob<{ default: unknown }>('../../data/registry.json');
...
async function bundlerLoad(
  ...
  if (!loader) {
    throw new DataLoaderError(`[DataLoader] File not found in bundle: '${relativePath}'`);
  }
...
async function fetchLoad(baseUrl: string, path: string): Promise<unknown> {
  ...
  try {
    ...
    throw new DataLoaderError(`[DataLoader] Network error loading '${path}': ${String(err)}`);
  }
  if (!res.ok) {
