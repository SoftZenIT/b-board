...
describe('StaleDetection', () => {
  ...
  describe('hasDisallowedAttributes', () => {
    it('returns true for a disabled input', () => {
      const input = document.createElement('input');
      input.disabled = true;
      expect(StaleDetection.hasDisallowedAttributes(input)).toBe(true);
    });
    ...
    it('returns false for an enabled, writable input', () => {
      const input = document.createElement('input');
      expect(StaleDetection.hasDisallowedAttributes(input)).toBe(false);
    });

    it('returns true for a disabled textarea', () => {
      const textarea = document.createElement('textarea');
      textarea.disabled = true;
      expect(StaleDetection.hasDisallowedAttributes(textarea)).toBe(true);
    });

    it('returns false for a plain element', () => {
      const div = document.createElement('div');
      expect(StaleDetection.hasDisallowedAttributes(div)).toBe(false);
    });
  });
});
