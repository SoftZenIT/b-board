...
describe('Data pipeline — real files, no mocks', () => {
  ...
  it('resolves all 8 language × layout combinations', async () => {
    ...
    for (const r of resolved) {
      ...
      // Verify production-ready features (Task 9.5 & 9.6)
      const firstKey = r.keyMap.values().next().value;
      expect(firstKey?.layers).toBeDefined();
      expect(firstKey?.layers?.base).toBeDefined();
      expect(firstKey?.layers?.shift).toBeDefined();
      expect(firstKey?.longPress).toBeInstanceOf(Array);
